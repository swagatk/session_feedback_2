...
document.getElementById('download-pdf-btn').onclick = () => {
    ...
    if (!window.jspdf || !window.jspdf.jsPDF) {
        alert('PDF Library (jspdf) not loaded. Please refresh.');
        return;
    }
    ...
    try {
        ...
        const surveyName = currentSurveyDetails.title?.split('|')[0]?.trim() || currentSurveyDetails.title || 'Feedback Report';
        ...
        doc.text(surveyName + ' - Feedback Report', 14, 20);

        // Metadata
        doc.setFontSize(11);
        doc.setTextColor(100);
        doc.text(`Session Date: ${surveyDate}`, 14, 28);
        doc.text(`Generated: ${new Date().toLocaleString()}`, 14, 34);

        // Stats
        doc.setFontSize(12);
        doc.setTextColor(0);
        doc.text(`Average Rating: ${avgRating}`, 14, 44);
        doc.text(`Total Responses: ${currentReportRows.length}`, 80, 44);

        // Prepare Table Data
        // Header row
        const tableHead = [currentReportColumns];
        
        // Body rows - mapped in order of columns
        const tableBody = currentReportRows.map(row => {
            return currentReportColumns.map(col => row[col] || '-');
        });
        ...
        // Save File
        const safeName = surveyName.replace(/[^a-z0-9]/gi, '_').substring(0, 30);
        doc.save(`Feedback_${safeName}.pdf`);
