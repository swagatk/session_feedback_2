...
document.getElementById('download-pdf-btn').onclick = async () => {
    ...
    // Create a visible overlay to guarantee rendering (fixes empty PDF issue)
    ...
    overlay.style.overflow = 'auto'; // allow scrolling if content is long
    ...
    container.innerHTML = `
        <h2 style="margin-top:0; color:#2c3e50; border-bottom: 2px solid #3498db; padding-bottom: 10px;">
            ${surveyName} - Feedback Report
        </h2>
        <div style="margin: 20px 0; font-size: 14px; background: #f8f9fa; padding: 15px; border-radius: 4px;">
            <div><strong>Session Date:</strong> ${surveyDate}</div>
            <div><strong>Generated On:</strong> ${new Date().toLocaleString()}</div>
            <div style="margin-top: 10px; font-size: 16px;">
                <strong>Average Rating:</strong> ${avgRating} | 
                <strong>Total Responses:</strong> ${currentReportRows.length}
            </div>
        </div>
            ...
            <thead>
                <tr style="background-color: #f1f1f1;">
                    ${currentReportColumns.map(col => 
                        `<th style="border: 1px solid #ddd; padding: 10px; text-align: left;">${col}</th>`
                    ).join('')}
                </tr>
            </thead>
            <tbody>
                ${currentReportRows.map(row => `
                    <tr>
                        ${currentReportColumns.map(col => 
        ...
        <div style="margin-top: 30px; font-size: 10px; color: #777; text-align: center; border-top: 1px solid #eee; padding-top: 10px;">
            Generated by Session Feedback App
        </div>
    `;
    ...
    // Give browser a moment to paint the overlay
    ...
    const opt = {
        ...
        html2canvas: { 
            scale: 2, 
            useCORS: true, 
            scrollY: 0, // Ensure we capture from top of overlay
